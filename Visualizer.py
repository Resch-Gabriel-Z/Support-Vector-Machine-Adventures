...
def visualize_svm(X, y, weights, bias):
    ...
    Args:
        ...
        bias: the bias of our model

    Returns: a plot that shows the hyperplane, and margins of our svm with the datapoints.
    ...
    hyperplane_l = draw_line(x_min, weights, bias, 0)
    hyperplane_r = draw_line(x_max, weights, bias, 0)

    # Get the lines for the gamma
    h_l_mgamma = draw_line(x_min, weights, bias, -1)
    h_r_mgamma = draw_line(x_max, weights, bias, -1)

    h_l_gamma = draw_line(x_min, weights, bias, 1)
    h_r_gamma = draw_line(x_max, weights, bias, 1)

    # Plot all of that
    ax.plot([x_min, x_max], [hyperplane_l, hyperplane_r], "r", alpha=0.5)
    ax.plot([x_min, x_max], [h_l_mgamma, h_r_mgamma], "m--", alpha=0.4)
    ax.plot([x_min, x_max], [h_l_gamma, h_r_gamma], "m--", alpha=0.4)
